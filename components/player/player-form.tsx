...
export function PlayerForm({
  ...
  // When the server fetched defaultValues from the database, in ms since
  // epoch -- lets the server tell a stale submission of this exact render
  // apart from a fresh one. Must come from the server (the page.tsx that
  // queried the player), NOT be computed client-side: a page restored from
  // cache -- the browser's back/forward cache, or Next re-mounting this
  // client component from a cached router payload -- still shows this same
  // stale render, but a client clock read at mount time would read "now,"
  // not "when this data was actually fetched," silently defeating the
  // staleness check it exists to support. See updatePlayerAdmin's
  // protectSince for where this gets used.
