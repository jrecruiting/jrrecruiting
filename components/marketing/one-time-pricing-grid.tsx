      ...
      {tiers.map((tier) => {
        ...
        return (
          <div key={tier.id} className="relative">
            {isBestValue && (
              <Badge className="absolute -top-3 left-1/2 z-10 -translate-x-1/2 bg-gold text-gold-foreground">
                Best Value
              </Badge>
            )}
            <Card
              className={`flex h-full flex-col border-border/60 ${isBestValue ? "border-gold/50" : ""}`}
            >
